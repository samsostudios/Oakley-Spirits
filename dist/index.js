...
(() => {
      ...
      verify = () => {
        class Verify {
          ...
          constructor() {
            ...
            this.submitButton = document.querySelector(".verify_submit");
          ...
          verifyAge() {
            ...
            const birthYear = this.inputs.map((element) => {
  ...
  // src/utils/forms/turnstile.ts
  init_live_reload();
  var SITEKEY = "0x4AAAAAADI7sqWWJ5PSmnBH";
  var _token = null;
  ...
  // src/utils/forms/mailchimp.ts
  init_live_reload();
  init_gsap();
  async function submitMailchimp(form, token) {
    const input = form.querySelector('input[data-name="Email"]');
    ...
    try {
      ...
      if (!response.ok) {
        const err = data.error.title === "Member Exists" ? "You are already a member!" : data.error.detail;
        showError(form, err);
      } else {
        showSuccess(form);
      }
  ...
  function showSuccess(form) {
    const wrap3 = form.closest(".form_wrap");
    gsapWithCSS.to(form, { display: "none" });
    gsapWithCSS.to(wrap3.querySelector(".form_success"), { display: "block" });
  }
  function showError(form, msg) {
    const wrap3 = form.closest(".form_wrap");
    const errorEl = wrap3.querySelector(".form_error");
    errorEl.children[0].innerHTML = msg;
    gsapWithCSS.to(errorEl, { display: "block" });
  }
  ...
  var FormHandler = class {
    ...
    handleMailchimp(form, token) {
      submitMailchimp(form, token);
    }
  };
  var formHandler = () => new FormHandler();
  var formHandler_default = formHandler;
