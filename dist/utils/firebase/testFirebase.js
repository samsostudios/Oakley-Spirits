...
(() => {
  ...
  // src/utils/firebase/getEntries.ts
  var getRaffleEntries = async () => {
  ...
  // src/utils/firebase/postEntries.ts
  var postRaffleEntry = async (email, name4) => {
    const entriesRef = ref(db, "raffleEntries");
    const snapshot = await get(entriesRef);
    if (snapshot.exists()) {
    ...
    console.log(`Successfully added ${name4} (${email}) to the raffle.`);
  };

  // src/utils/firebase/testFirebase.ts
  (async () => {
